...
Architecture-documentation validation passed with a temporary SQLite override: `python manage.py check`, `python manage.py makemigrations --check`, 355 tests with 7 PostgreSQL-only concurrency tests skipped, `ruff check .`, and `git diff --check`. The document contains 18 Mermaid blocks in total (16 in the new suite plus 2 pre-existing diagrams), with balanced fences and a manual syntax/consistency review. Mermaid CLI was not installed, and an on-demand fetch was unavailable in the restricted environment, so no automated Mermaid render was produced.
